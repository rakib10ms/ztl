...
function EditServiceLogo() {
    ...
    const { id } = useParams();
    useEffect(() => {
        axios.get(`/edit-service-logo/${id}`).then(res => {
            if (res.data.status == 200) {
                console.log('res data', res.data)
                setPicture(res.data.service_logo.service_logo);
                setImage(res.data.service_logo.service_logo);
            }
        })
    ...
    const updateServiceLogo = (e) => {
        ...
        if (image == '') {
            ...
            axios.post(`/update-service-logo/${id}`, formData).then(res => {
                if (res.data.status == 200) {
                        ...
                        <div className="col-md-12 mt-3">
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title"> Edit Service Logo
                                        <Link to={'/service-logo'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                    </h6>
                                </div>
                                <div className="card-body">
                                    <form id="EarCanalForm" onSubmit={updateServiceLogo} encType="multipart/form-data">
                                        <div className="card-body">
                                            ...
                                            <div className="col-md-12">
                                                ...
                                                </div>
                                                {
                                                    picture == '' ? <div className="form-group" style={{ width: '100px', height: '90px' }}>
                                                        <img className="playerProfilePic_home_tile" src={`${global.img_url}/images/service_logo/${image}` } style={{ width: '100px', height: '90px' }}></img>
                                                    </div>
                                                        :
