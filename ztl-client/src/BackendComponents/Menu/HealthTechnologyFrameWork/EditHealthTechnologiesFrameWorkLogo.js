...
function EditHealthTechnologiesFrameWorkLogo() {
    ...
    const { id } = useParams();
    useEffect(() => {
        axios.get(`/edit-health-technologies-framework-logo/${id}`).then(res => {
    ...
    const updatehealth = (e) => {
        ...
        if (image == '') {
            ...
            axios.post(`/update-health-technologies-framework-logo/${id}`, formData).then(res => {
                if (res.data.status == 200) {
                    Swal.fire(res.data.message, '', 'success')
                    navigate('/health-technologies-framework-logo');
...
export default EditHealthTechnologiesFrameWorkLogo
