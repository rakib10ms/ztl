...
function WebDevelopmentCard() {
    ...
    const deleteJobDescription = (e, id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete-web-development-card/${id}`).then(res => {
                    if (res.data.status === 200) {
    ...
    const columns = [
        ...
        {
            title: "Action", field: "patient", render: (row) => <div className='d-flex'><Link to={`/edit-web-development-card/${row.id}`} class="btn btn-info btn-sm action-btn"><i class="fas fa-edit"></i></Link>
                        ...
                        <div className="col-md-12 mt-3">
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title">Web Development Card
                                        <Link to='/add-web-development-card' className="btn btn-primary btn-sm float-end"> Add </Link>
                                    </h6>
                                </div>
