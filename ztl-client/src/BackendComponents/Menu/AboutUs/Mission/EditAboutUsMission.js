...
const navigate=useNavigate();
    ...
    const handleInput = (e) => {
        setaboutMission({
            ...aboutMission, [e.target.name]: e.target.value
        });
    }
    const { id } = useParams();
    useEffect(() => {
        axios.get(`/edit-about-us-mission/${id}`).then(res => {
            if (res.data.status == 200) {
                setaboutMission({
                    mission_description: res.data.about.mission_description,
                });
    ...
    const updateErp = (e) => {
        ...
        formData.append('mission_description', aboutMission.mission_description);
        formData.append('image', image);


        if (aboutMission.mission_description == '' || image == '') {
            ...
            axios.post(`/update-about-us-mission/${id}`, formData).then(res => {
                if (res.data.status == 200) {
                    ...
                    setaboutMission({ ...aboutMission, error_list: res.data.errors });
                            ...
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title"> Edit Mission Description & Image
                                        <Link to={'/about-us-mission'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                    </h6>
                                </div>
                                        ...
                                        <div className="card-body">
                                            ...
                                            <div className="col-md-12">
                                                ...
                                                <div className="form-group">
                                                    ...
                                                    <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" value={aboutMission.mission_description} className="form-control" name="mission_description" onChange={handleInput} ></textarea>
                                                    {/* <span className="text-danger">{heading.error_list.mission_description}</span> */}
                                                ...
                                                <div className="form-group">
                                                    ...
                                                    {/* <input type="file" value={aboutMission.client_image} className="form-control" name="client_image" onChange={(e) => { setImage(e.target.files[0]) }} /> */}
