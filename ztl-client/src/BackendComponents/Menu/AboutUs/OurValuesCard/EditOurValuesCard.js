...
function EditOurValuesCard() {
    ...
    const handleInput = (e) => {
        setourValues({
            ...ourValues, [e.target.name]: e.target.value
        });
    }
    const { id } = useParams();
    useEffect(() => {
        axios.get(`/edit-our-values-card/${id}`).then(res => {
            if (res.data.status == 200) {
                setourValues({
                    title: res.data.value.title,
                    short_description: res.data.value.short_description,
                });
                setImage(res.data.value.image);
            }
        })
    ...
    const updateourValues = (e) => {
        e.preventDefault();
        const formData = new FormData();
        formData.append('title', ourValues.title);
        formData.append('short_description', ourValues.short_description);
        ...
        if (ourValues.title == '' || ourValues.short_description == '' || image == '') {
            ...
            axios.post(`/update-our-values-card/${id}`, formData).then(res => {
                if (res.data.status == 200) {
                    Swal.fire(res.data.message, '', 'success')
                    navigate('/our-values-card')
                    setourValues({
                        title: "",
                        short_description: "",
                    });
                    ...
                    setourValues({ ...ourValues, error_list: res.data.errors });
                        ...
                        <div className="col-md-12 mt-3">
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title"> Edit Our Values Card
                                        <Link to={'/our-values-card'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                    </h6>
                                </div>
                                <div className="card-body">
                                    <form id="EarCanalForm" onSubmit={updateourValues} encType="multipart/form-data">
                                        <div className="card-body">
                                            ...
                                            <div className="col-md-12">
                                                ...
                                                <div className="form-group">
                                                    ...
                                                    <input type="text" value={ourValues.title} className="form-control" name="title" onChange={handleInput} />
                                                ...
                                                <div className="form-group">
                                                    ...
                                                    <textarea class="form-control" id="exampleFormControlTextarea1" value={ourValues.short_description} name="short_description" rows="3" onChange={handleInput} ></textarea>
                                                    {/* <span className="text-danger">{heading.error_list.title}</span> */}
...
export default EditOurValuesCard
