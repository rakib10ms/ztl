...
function OurValuesCard() {
    ...
    const deleteServiceType = (e, id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete-our-values-card/${id}`).then(res => {
                    if (res.data.status === 200) {
    ...
    const columns = [
        ...
        {
            title: "Action", field: "patient", render: (row) => <div className='d-flex'><Link to={`/edit-our-values-card/${row.id}`} class="btn btn-info btn-sm action-btn"><i class="fas fa-edit"></i></Link>
                        ...
                        <div className="col-md-12 mt-3">
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title">Our Values  Card
                                        <Link to='/add-our-values-card' className="btn btn-primary btn-sm float-end"> Add Our Values Card</Link>
                                    </h6>
                                </div>
