...
function OurWorldWideProject() {
    ...
    const deleteJobDescription = (e, id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete-our-world-wide-project/${id}`).then(res => {
                    if (res.data.status === 200) {
    ...
    const columns = [
        ...
        {
            title: "Action", field: "patient", render: (row) => <div className='d-flex'><Link to={`/edit-our-world-wide-project/${row.id}`} class="btn btn-info btn-sm action-btn"><i class="fas fa-edit"></i></Link>
                            ...
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title">Job Description
                                        <Link to='/add-our-world-wide-project' className="btn btn-primary btn-sm float-end"> Add </Link>
                                    </h6>
                                </div>
...
export default OurWorldWideProject;
