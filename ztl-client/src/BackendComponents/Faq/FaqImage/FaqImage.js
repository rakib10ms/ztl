...
function FaqImage() {
    ...
    const deleteFaqImage = (e, id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete-faq-image/${id}`).then(res => {
                    if (res.data.status === 200) {
    ...
    const columns = [
        ...
        {
            title: "Action", field: "patient", render: (row) => <div className='d-flex'><Link to={`/edit-faq-image/${row.id}`} class="btn btn-info btn-sm action-btn"><i class="fas fa-edit"></i></Link>
                            ...
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title">Faq Image
                                        <Link to='/add-faq-image' className="btn btn-primary btn-sm float-end"> Add Faq Image</Link>
                                    </h6>
                                </div>
