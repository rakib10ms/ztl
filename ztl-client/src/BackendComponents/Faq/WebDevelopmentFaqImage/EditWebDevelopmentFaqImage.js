...
function EditWebDevelopmentFaqImage() {
    ...
    const { id } = useParams();
    useEffect(() => {
        axios.get(`/edit-web-development-faq-image/${id}`).then(res => {
    ...
    const updateFaqImage = (e) => {
        ...
        if (image == '') {
            ...
            axios.post(`/update-web-development-faq-image/${id}`, formData).then(res => {
                if (res.data.status == 200) {
                    Swal.fire(res.data.message, '', 'success')
                    navigate('/web-development-faq-image')
                            ...
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title"> Edit Faq Image
                                        <Link to={'/web-development-faq-image'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                    </h6>
                                </div>
...
export default EditWebDevelopmentFaqImage
